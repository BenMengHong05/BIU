...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM instagrams WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id', $id);
    $stmt->execute();
    $instagrams = $stmt->fetch(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFilePath)) {
            ...
            $imagePathForDB = $instagrams['image']; // Fallback to existing image path
        }
    } else {
        // No new image uploaded, keep the existing path
        $imagePathForDB = $instagrams['image'];
    }

    // Prepare the SQL statement for update
    $sql = "UPDATE instagrams SET  image = :image WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    ...
    if ($stmt->execute()) {
        header("Location: instagram_table.php?message=updated"); 
        exit;
    } else {
...
?>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Edit instagrams Item</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                    <li class="breadcrumb-item active">Edit instagrams Item</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
        ...
        <section class="section dashboard">
            <div class="row">
                <div class="col-lg-12">
                    <form action="instagram_edit.php?id=<?php echo htmlspecialchars($id); ?>" method="POST" enctype="multipart/form-data">
